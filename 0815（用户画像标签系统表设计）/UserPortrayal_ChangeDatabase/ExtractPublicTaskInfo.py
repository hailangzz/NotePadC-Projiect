...
def ExtractPublicTaskInfo(MysqlObject):
    try:
        MysqlCommand = "select labelname,status,DATE_FORMAT(createtime,'%%Y-%%m-%%d %%H:%%i:%%S'),DATE_FORMAT(endtime,'%%Y-%%m-%%d %%H:%%i:%%S') from %s.label_group_personas where createtime>'2018-05-13' and status=5;" % MysqlObject._UseDatabase
        ...
        for PublicBatchInfoTupleIndex in range(len(PublicBatchInfoTupleList)):
            if PublicBatchInfoTupleIndex+1 not in GV.ExtractPublicTaskInfoDict['ExtractTaskID']:
                ...
                GV.ExtractPublicTaskInfoDict['ExtractTaskID'][PublicBatchInfoTupleIndex + 1]['status']=PublicBatchInfoTupleList[PublicBatchInfoTupleIndex][1]
                GV.ExtractPublicTaskInfoDict['ExtractTaskID'][PublicBatchInfoTupleIndex + 1]['createtime']=PublicBatchInfoTupleList[PublicBatchInfoTupleIndex][2]
                GV.ExtractPublicTaskInfoDict['ExtractTaskID'][PublicBatchInfoTupleIndex + 1]['endtime']=PublicBatchInfoTupleList[PublicBatchInfoTupleIndex][3]
